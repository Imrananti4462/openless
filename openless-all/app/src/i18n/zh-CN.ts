...
export const zhCN = {
  ...
  translation: {
    ...
    howto: {
      ...
      step2: '按一下录音快捷键（当前：{{trigger}}）开始录音。',
      step3: '在录音过程中任意时刻按一下 Shift 即可，不需要按住；可以在开口前、说到一半或快结束时按。',
      step4: '再按一次录音快捷键停止录音。',
      step5: '系统会把转写交给大模型翻译成上面选的目标语言，然后插入到一开始那个输入框光标位置。',
      indicatorTitle: '怎么知道翻译模式生效了',
      indicatorDesc: '一旦按下 Shift，屏幕底部录音胶囊上方会出现蓝色「正在翻译」提示，并持续到本次插入完成。',
      fallbackTitle: '安全兜底',
      fallbackDesc: '翻译模式选「不启用」时 Shift 不会触发翻译；如果大模型调用失败，会回退插入原始转写，不会丢字。',
    },
  },
  selectionAsk: {
