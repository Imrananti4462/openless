...
export const zhCN = {
  ...
  selectionAsk: {
    ...
    title: '划词追问',
    desc: '选中任意 app 里的一段文字，按下快捷键，对着麦克风口头提问，AI 答案会以浮窗显示在屏幕底部录音胶囊正上方。',
    statusEnabled: '已启用',
    statusDisabled: '未启用',
    hotkey: {
      title: '触发快捷键',
      desc: '选了某组按键后，在任意 app 选中文字时按下，浮窗即可弹出并开始录音；再按一次停止录音并生成答案。选「不启用」则关闭整个功能。',
      optionDisabled: '不启用',
      chordWarning: '⚠️ 双修饰键 chord（Cmd+Option / Ctrl+Alt）触发条件：两个修饰键同时按下、不按其他键、再松开两个键。优点是按键少；缺点是误触率比 3 键组合高（你想按 Cmd+Option+某字母 但还没按字母时容易触发）。后端 chord 监听需要 v1.2.9+ 才支持，更早版本请选 3 键组合。',
    },
    history: {
      title: '保存历史',
    ...
    howto: {
      title: '使用方法',
      step1: '在任意 app（浏览器、Mail、IDE、PDF reader…）选中一段文字。',
      step2: '按下「{{hotkey}}」——浮窗在屏幕底部弹出，同时进入语音录音状态。',
      step3: '对着麦克风提问——比如"这是什么意思 / 用大白话解释 / 跟 X 有什么区别 / 把它拆成几个要点"。',
      step4: '再按一次「{{hotkey}}」停止录音。系统会把「你选中的文字 + 你的语音问题」一起送给大模型生成答案。',
      step5: '答案以 markdown 格式显示在浮窗里。读完按 Esc 或点击外部即可关闭；右上角 📌 钉住则会一直保留直到你手动关。',
      windowTitle: '浮窗位置 + 生命周期',
      windowDesc: '浮窗默认在录音胶囊正上方 8px 处，宽 380×高 280。30 秒未操作自动关闭（钉住的不算）。窗口有自己的滚动条，长答案可以滚动阅读。',
      privacyTitle: '隐私契约',
