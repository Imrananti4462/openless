...
OpenLess 是一个原生 macOS 菜单栏语音输入层：把光标放在任意输入框，按下全局快捷键说话，OpenLess 会录音、转写、轻度整理，并把结果插入当前输入位置。插入失败时会自动复制到剪贴板，尽量保证“用户说过的话不丢”。
...
- 支持底部微型状态胶囊、菜单栏控制、设置页、历史记录、剪贴板兜底。
...
4. 在菜单栏中打开设置，填写火山引擎 ASR 和 Ark 凭据。
...
设置页里的“词典”标签可以添加和管理词条；“效果”标签会展示口述时长、估算节省的打字时间、平均口述字数、速度提升和词典参与记录。
